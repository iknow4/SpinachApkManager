...
public class ToSdCardActivity extends UniversalActivity
{
	class AnalysisAppTask extends AsyncTask
	{
		...
		{
			Log.i("ToSdCardActivity", "doInBackground.......................");
		...
		protected void onPostExecute(String s)
		{
			...
			Log.i("ToSdCardActivity", "onPostExecute sendEmptyMessage-->>HANDLER_ANALYSIS_INSTALLED_APP_FINISH_MSG");
		...
		protected void onPreExecute()
		{
			...
			analysisProgressDialog = new ProgressDialog(ToSdCardActivity.this);
	...
	public static final int INSTALL_LOCATION_DEFAULT = 3;
	private static final String TAG = "ToSdCardActivity";

	private static final int CONFIRM_MOVE_TO_SDCARD_DIALOG = 10001;
	...
	private void analysisApplications()
	{
		...
		Iterator iterator;
		Log.i("ToSdCardActivity", "analysisApplications....................");
		if (phoneList == null)
		{
			Log.i("ToSdCardActivity", "phoneList == null");
		...
		} else
		{
			...
			phoneList.clear();
			Log.i("ToSdCardActivity", "phoneList.clear()");
		}
	try{
		PackageManager packagemanager = getPackageManager();
		//iterator = packagemanager.getInstalledPackages(0).iterator();
		List<PackageInfo> packageInfoList = getPackageManager().getInstalledPackages(0); 
		Log.i("ToSdCardActivity", "PackageInfo............................");
		//PackageInfo packageinfo;
		String packageName;
		boolean flag3;
		for(PackageInfo packageinfo : packageInfoList){
			Log.i("ToSdCardActivity", "packageInfoList............................");
			...
			if(activityInfo!=null){
				if ((packageinfo.applicationInfo.flags & 1) <= 0){
					...
					Log.i("ToSdCardActivity", "packageinfo.applicationInfo.flags & 1) <= 0");
						                ...
						                for (int j = 0; j < xml.getAttributeCount(); j++) 
						                {
						                    if (xml.getAttributeName(j).matches("installLocation")) 
						                    {
						                        switch (Integer.parseInt(xml.getAttributeValue(j))) 
						                        {
						                            ...
						                            case INSTALL_LOCATION_AUTO:
						                                // Do stuff
						                            	Log.i(TAG,"INSTALL_LOCATION_AUTO:packageName = "+packageName);
						                                	distributeAppToList(packageinfo, s4, INSTALL_LOCATION_AUTO, packagemanager);
						                                	break;
						                            case INSTALL_LOCATION_INTERNAL_ONLY:
						                                // Do stuf
						                            	Log.i(TAG,"INSTALL_LOCATION_INTERNAL_ONLY:packageName = "+packageName);
											...
											mHandler.sendMessage(message1);
						                            ...
						                            case INSTALL_LOCATION_PREFER_EXTERNAL:
						                                // Do stuff
						                            		Log.i(TAG,"INSTALL_LOCATION_PREFER_EXTERNAL : packageName = "+packageName);
						                               		distributeAppToList(packageinfo, s4, INSTALL_LOCATION_PREFER_EXTERNAL, packagemanager);
						                                	break;
						                            default:
						                                // Shouldn't happen
						                                // Do stuff
						                                		distributeAppToList(packageinfo, s4, INSTALL_LOCATION_DEFAULT, packagemanager);
						                            		Log.i(TAG,"default:packageName = "+packageName);
						                                	break;
						                        }
						                      //  break ;
						                    }
			...
			{
				Log.i("ToSdCardActivity", " activityInfo = null  ..................");	
			}
			...
			Log.i("ToSdCardActivity", " ToSdCardActivity   Exception ..................");	
	}
	}
	...
	{
		Log.i("ToSdCardActivity", "distributeAppToList  start");
		...
		aapplication.setSizeValue(size.length());
		aapplication.setPackageName(pkgname);
		aapplication.setVersionName(versionname);
		Log.i("ToSdCardActivity", "SdCardChecker -> pkgname = "+pkgname);
		if ((packageinfo.applicationInfo.flags & ApplicationInfo.FLAG_EXTERNAL_STORAGE) != 0){
			;  
		}
		else if(SdCardChecker.check(packageinfo.applicationInfo))
		{	
			
			Log.i("ToSdCardActivity", "distributeAppToList?");
			phoneList.add(aapplication);
		}
	...
	private void setupViews()
	{
		...
		res = resources;
		String s = res.getString(R.string.move_to_sdcard);
		...
		mainListView.setOnItemClickListener(new android.widget.AdapterView.OnItemClickListener(){
			...
			public void onItemClick(AdapterView adapterview, View view, int i,long arg3) {
				...
				//Utils.startInstalledAppDetailsAction(pkgName, ToSdCardActivity.this);
				...
				AApplication aapplication = (AApplication)phoneList.get(i);
					...
					do
					{
						if (!iterator.hasNext())
						{
							...
							if (tempAppList.size() == 0)
								...
								showDialog(CONFIRM_MOVE_TO_SDCARD_DIALOG);
							return;
						}
	...
	{
		Log.i("ToSdCardActivity", "onCreate");
		super.onCreate(bundle);
		//requestWindowFeature(1);
		//setContentView(R.layout.to_rom);
		setupViews();
		//mCanBeOnSdCardChecker = new SdCardChecker();
		//mCanBeOnSdCardChecker.init(ToSdCardActivity.this);
		needRefresh = true;
	...
	{
		super.onResume();
		Log.i("ToSdCardActivity", "onResume");
		SharedPreferences sh = getSharedPreferences("is from about activity", 0);
        isfromAboutActivity = sh.getBoolean("isneedrefresh", false);
		...
		if (needRefresh)
		{
			Log.i("ToSdCardActivity", "needRefresh = true");
		...
		public void handleMessage(Message message)
		{
			switch(message.what)
			{
				case HANDLER_ANALYSIS_INSTALLED_APP_PROGRESS_MSG:
					...
					analysisProgressDialog.setMessage(s3);
					break;
				case HANDLER_ANALYSIS_INSTALLED_APP_FINISH_MSG:
					analysisProgressDialog.dismiss();
					
					int sdPhoneListSize = phoneList.size();
						
					//if (sdPhoneListSize> 0)
					//{
						Log.i("ToSdCardActivity", "ToSdCardActivity HANDLER_ANALYSIS_INSTALLED_APP_FINISH_MSG");
						phoneListAdapter = new AppListAdapter(ToSdCardActivity.this, phoneList);
						
						mainListView.setAdapter(phoneListAdapter);
	...
	//“Ï≤Ω¥¶¿Ì
	class MoveToSdCardTask extends AsyncTask
	{
		...
		private MoveToSdCardTask()
		{
			
			super();
		}

		MoveToSdCardTask(MoveToSdCardTask movetosdcardtask)
		{
			this();
		}
	}
	...
	protected Dialog onCreateDialog(int i)
	{
		...
		switch(i)
		{
			case NO_SELECTED_ITEM_DIALOG:
				dialog = buildNSUD();
				break;
			case CONFIRM_MOVE_TO_SDCARD_DIALOG: 
				dialog =  buildCOUD();
				break;
		}
		return dialog;
	...
	private class mlistener2 implements android.content.DialogInterface.OnClickListener
	{
		public void onClick(DialogInterface dialoginterface, int i)
		{
			MoveToSdCardTask task = new MoveToSdCardTask(null);
			String as[] = new String[0];
			task.execute(as);
			dialoginterface.dismiss();
		}
